...
var _filter=new URLSearchParams(location.search).get('status')||'';
function act(a,d){d.action=a;var f=new FormData;for(var k in d)f.append(k,d[k]);fetch('api.php',{method:'POST',headers:{'X-Requested-With':'XMLHttpRequest'},body:f}).then(function(r){return r.json()}).then(function(j){if(j.ok){var c=document.getElementById(a.includes('copy')?'c-'+(d.copy_id||''):'a-'+(d.asset_id||''));if(c){var newStatus=a.includes('approve')?'approved':a.includes('reject')?'rejected':a.includes('feedback')?'feedback':'done';if(_filter&&_filter!==newStatus){c.style.transition='opacity .2s,max-height .3s,margin .3s,padding .3s';c.style.opacity='0';c.style.maxHeight=c.offsetHeight+'px';setTimeout(function(){c.style.maxHeight='0';c.style.margin='0';c.style.padding='0';c.style.overflow='hidden';setTimeout(function(){c.remove()},300)},200)}else{var lbl=newStatus.charAt(0).toUpperCase()+newStatus.slice(1);var clr=newStatus==='approved'?'var(--green)':newStatus==='rejected'?'var(--red)':'var(--orange)';var badge=c.querySelector('.badge-tr')||c.querySelector('.ch span:last-child span:last-child');if(badge){badge.textContent=lbl;badge.style.color=clr}c.style.opacity='.5'}}}else toast(j.error||'Failed','e')}).catch(function(){toast('Network error','e')})}
